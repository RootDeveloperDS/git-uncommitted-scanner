...
### UX Upgrade - TUI Dynamic Status Bar Coloring
- Added distinct CSS classes (`.status-loading`, `.status-success`, `.status-warning`) to the Textual TUI `#status-bar` to provide better visual cues during scanning states.
- Replaced implicit static styles with dynamic class toggling using `add_class()` and `remove_class()`.
