...
def scan(
    ...
    export: Optional[str] = typer.Option(None, "--export", help="Export scan results as JSON or CSV to specified file path (inferred from extension)")
):
    ...
    if export:
        ...
        if export_file.suffix.lower() == ".csv":
            with export_file.open("w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=["path", "branch", "modified", "untracked", "last_commit", "last_commit_timestamp"])
                writer.writeheader()
